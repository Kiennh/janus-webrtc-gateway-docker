...
run: 
	@docker run --rm -p 80:80 -p 8088:8088 -p 8188:8188  -v /root/kien/transcoder/docker-gstreamer:/root/tmp --name="janus" -it -t atyenoria/$(TEMPLATE_NAME)

run-host: 
	@docker run --net=host  -v /root/kien/transcoder/docker-gstreamer:/root/tmp --name="janus" -it -t atyenoria/$(TEMPLATE_NAME)
